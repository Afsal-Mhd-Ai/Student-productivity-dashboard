...
var AI_FEED = [
  ...
  [
    ...
    {topic:'LLM on Mobile',       source:'Meta AI · 2025',   desc:'Llama 3 running fully on-device on Android — 7B model at 30 tok/s with no internet required.'},
  ],
  [
    ...
    {topic:'India AI Mission',    source:'GoI · 2025',       desc:'10,000 Cr IndiaAI mission announced — compute infra, startup grants, GPU access for students.'},
  ],
];
...
function init() {
  ...
  document.getElementById('stat-focus').textContent = (state.pomoSessions * 25 / 60).toFixed(1) + 'h';
...
function renderTasks() {
  ...
  if (todayTasks.length === 0) {
    list.innerHTML =
      '<div style="text-align:center;padding:20px 0;color:var(--muted);font-size:12px;font-family:var(--mono)">' +
      'No tasks yet. Add your first task! 💪</div>';
  } else {
    list.innerHTML = todayTasks.map(function(t) {
      return (
        '<div class="task-item" onclick="toggleTask(\'' + t.id + '\')">' +
          ...
          '<div onclick="event.stopPropagation();deleteTask(\'' + t.id + '\')" ' +
               'style="color:var(--muted);font-size:12px;cursor:pointer;padding:2px 6px;border-radius:4px;" ' +
               'title="Delete">✕</div>' +
        '</div>'
      );
    }).join('');
  ...
  var pending = todayTasks.filter(function(t) { return !t.done; }).length;
  ...
  document.getElementById('exam-list').innerHTML = exams.map(function(e) {
    ...
    return (
      '<div class="exam-item">' +
        ...
        '</div>' +
        '<div class="exam-days">' +
          '<div class="n" style="color:' + color + '">' + (diff > 0 ? diff : '✓') + '</div>' +
          '<div class="u">days left</div>' +
        '</div>' +
      '</div>'
    );
  }).join('');
      ...
      return (
        '<div class="ai-item">' +
          '<div class="ai-item-head">' +
            ...
            '<div class="ai-topic">' + item.topic + '</div>' +
          '</div>' +
